...
class StateGenerator {
	...
	private def String generateTimeout() '''
		if («generateGlobalTime» - «process.generateTimeoutName») >= «IF state.timeout.variable !== null»«statementList.generateVar(state.timeout.variable)»«ELSE»«state.timeout.const.generateConstant»«ENDIF»:
