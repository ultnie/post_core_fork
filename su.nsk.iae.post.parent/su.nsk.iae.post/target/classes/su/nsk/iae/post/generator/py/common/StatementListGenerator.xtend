...
class StatementListGenerator {
	...
	def String generateProcessStatus(ProcessStatusExpression exp) {
		if (exp.active) {
			return '''((processesDict['«exp.process.name»'].curState != processesDict['«exp.process.name»'].States.Stop) and (processesDict['«exp.process.name»'].curState != processesDict['«exp.process.name»'].States.Error))'''
		} else if (exp.inactive) {
			return '''((processesDict['«exp.process.name»'].curState == processesDict['«exp.process.name»'].States.Stop) or (processesDict['«exp.process.name»'].curState == processesDict['«exp.process.name»'].States.Error))'''
		} else if (exp.stop) {
			return '''(processesDict['«exp.process.name»'].curState == processesDict['«exp.process.name»'].States.Stop)'''
		}
		return '''(processesDict['«exp.process.name»'].curState == processesDict['«exp.process.name»'].States.Error)'''
	}
	...
	private def initStatementGenerators() {
		return Arrays.asList(
			...
			new InlineStatementGenerator(program, process, state, this)
		)
	}
